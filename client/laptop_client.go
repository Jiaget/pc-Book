...
// CreateLaptop creates a laptop RPC
func (laptopClient *LaptopClient) CreateLaptop(laptop *pb.Laptop) {
	...
	if err != nil {
		...
		if ok && st.Code() == codes.AlreadyExists {
			// just print out the error, not serious
...
func (laptopClient *LaptopClient) SearchLaptop(filter *pb.Filter) {
	...
	stream, err := laptopClient.service.SearchLaptop(ctx, request)
	if err != nil {
	...
	for {
		...
		log.Print(" + price: ", laptop.GetPriceRmb(), "RMB")
...
func (laptopClient *LaptopClient) UploadImage(laptopID string, imagePath string) {
	...
	if err != nil {
		log.Fatal("cannot loadup image: ", err)
	}
...
func (laptopClient *LaptopClient) RateLaptop(laptopIDs []string, scores []float64) error {
	...
	for i, laptopID := range laptopIDs {
		...
		err := stream.Send(req)
		if err != nil {
			// second param is the real err, to get the exact error
